...
JUICE_GLASS = 295
MEDIUM_EGG = 58
FOURTEEN_INCH_PIZZA = 250 # => this is an estimate
PINT = 473
CFPCF = {
	...
	almonds: [5.85, 0.52, 0.205, 0.205, 0.118], 
	...
	ayran_mint_youghurt_drink: [0.32, 0.017, 0.017, 0.024, 0],
	...
	couscous: [3.76, 0.0064, 0.1276, 0.7743, 0.05],
	...
	peanut_butter_african: [4.78, 0.53, 0.266, 0.166, 0],
	...
	protein_syntha_6: [3.93, 0.14, 0.48, 0.13, 0.12],
	...
	swee_po: [0.9, 0.0003, 0.02, 0.206, 0.03], 
	sweetcorn: [0.73, 0.014, 0.024, 0.11, 0.027],
	stout: [0.52, 0, 0.0053, 0.0052, 0],
	...
	v_brownie: [3.94, 0.183, 0.042, 0.577, 0.028],
	...
	v_ice_cream: [1.95, 0.119, 0.034, 0.195, 0],
...
def calories_calculator(ingredients)
	macros = Array.new(5, 0)
	ingredients.each do |food, weight|
		macros.each_with_index do |macro, idx|
			macros[idx] += CFPCF[food][idx]*weight
		end
	end
	macros.map(&:round)
end

def gram_maker(ingredients, weight)
	calories_calculator(ingredients).map {|num| (num.to_f/weight).round(3)}
end
...
wed_ric_ingredients = {
	...
	cherry_tomatoes: 257,
	cucumber: 179,
	chopped_tomatoes_co_op: 400,
	parsley: 97,
}

wed_ric_weight = 2489 + 1693 + 342
CFPCF[:wed_ric] = gram_maker(wed_ric_ingredients, wed_ric_weight)

wed_cous_ingredients = {
	couscous: 280,
	chicken_breast: 460,
	veggie_stock_cubes: 20,
	sweetcorn: 285,
...
wed_cous_weight = 2177
CFPCF[:wed_cous] = gram_maker(wed_cous_ingredients, wed_cous_weight)

today_input = {
	wed_ric: 815 + 876,
	egg: MEDIUM_EGG,
	nectarine: 59,
	peanut_butter_african: 42,
	oatcakes: 88,
	almonds: 7,
	mayonaise: 31,
	cherry_tomatoes: 271,
	pickled_onions: 55,
	ayran_mint_youghurt_drink: 101,
	orange_juice_tropicana: 146,
	apricot: APRICOT,
	v_ice_cream: 100,
	protein_syntha_6: 45,
}

today_macros = calories_calculator(today_input)

 puts "Thursday 16th June:
...
#puts "Each gram in tuesday's cook has 
#{mon_por_grams[0]} calories, 
#{mon_por_grams[1]}g fat 
#{mon_por_grams[2]}g protein, 
#{mon_por_grams[3]}g carbs and  
#{mon_por_grams[4]}g fibre" 
